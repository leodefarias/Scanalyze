...
public class MicroscopyImageDAO {
    ...
    public boolean insert(MicroscopyImage image) throws SQLException {
        String sql = "INSERT INTO " + DatabaseConfig.TABLE_MICROSCOPY_IMAGES +
                " (ID, IMAGE_ID, SAMPLE_ID_FK, MICROSCOPE_ID_FK, ARQUIVO, IMAGEM_BLOB, FORMATO, " +
        ...
        try {
            ...
            // Salva o BLOB se disponível
            if (image.hasImagemBlob()) {
                stmt.setBytes(4, image.getImagemBlob());
            } else {
                stmt.setNull(4, java.sql.Types.BLOB);
            }

            stmt.setString(5, DatabaseConfig.DEFAULT_IMAGE_FORMAT);
            stmt.setTimestamp(6, Timestamp.valueOf(image.getDataCaptura()));
            stmt.setString(7, "Sistema");

            int rowsAffected = stmt.executeUpdate();
            ConnectionFactory.commitTransaction(connection);

            System.out.println("Imagem inserida com sucesso: " + image.getId() +
                    (image.hasImagemBlob() ? " (com BLOB: " + image.getImagemBlobSize() + " bytes)" : " (sem BLOB)"));
    ...
    private MicroscopyImage extractImageFromResultSet(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            // Algumas versões do schema utilizam NOME_ARQUIVO
        ...
        // Criar imagem
        MicroscopyImage image = new MicroscopyImage(imageId, arquivo, null, dataCaptura);

        // Tenta recuperar o BLOB se disponível
        try {
            Blob blob = rs.getBlob("IMAGEM_BLOB");
            if (blob != null) {
                int blobLength = (int) blob.length();
                byte[] imageBytes = blob.getBytes(1, blobLength);
                image.setImagemBlob(imageBytes);
            }
        } catch (SQLException e) {
            // Campo BLOB pode não existir em versões antigas do schema
            System.out.println("BLOB não disponível para imagem " + imageId);
        }
    ...
    /**
     * Recupera apenas a imagem BLOB por ID (útil para exibição).
     */
    public byte[] getImageBlobById(String imageId) throws SQLException {
        String sql = "SELECT IMAGEM_BLOB FROM " + DatabaseConfig.TABLE_MICROSCOPY_IMAGES +
                " WHERE IMAGE_ID = ?";
        ...
        try {
            ...
            System.err.println("Erro ao recuperar BLOB da imagem: " + e.getMessage());
            throw e;
        } finally {
            if (rs != null) rs.close();
            if (stmt != null) stmt.close();
