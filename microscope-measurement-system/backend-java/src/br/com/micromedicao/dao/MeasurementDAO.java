...
public class MeasurementDAO {
    ...
    private Measurement extractMeasurementFromResultSet(ResultSet rs) throws SQLException {
        ...
        double scalePixelsPerUm = 0.0;

        try {
            arquivo = rs.getString("IMAGE_ARQUIVO");
        } catch (SQLException ignored) {
            // Algumas estruturas utilizam NOME_ARQUIVO em vez de ARQUIVO
            try {
        ...
        try {
            areaPixels = rs.getDouble("AREA_PIXELS");
        } catch (SQLException ignored) {
            areaPixels = 0.0;
        }

        try {
            scalePixelsPerUm = rs.getDouble("SCALE_PIXELS_PER_UM");
        } catch (SQLException ignored) {
            scalePixelsPerUm = 0.0;
        }

        Measurement measurement = new Measurement(measurementId, sample, area, areaPixels, scalePixelsPerUm, dataHora, image);
        measurement.setImagem(image);
        measurement.setAreaPixels(areaPixels);
        measurement.setScalePixelsPerUm(scalePixelsPerUm);
        return measurement;
    }
