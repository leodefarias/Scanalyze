 ...
 * Esta classe inicializa o sistema com integração ao banco Oracle,
 * testa a persistência de dados e demonstra o funcionamento completo
 * com recuperação automática após reinicialização.
 *
 * @author Sistema de Micromedição
 * @version 2.0 - Integração com Oracle Database
 */
public class App {
    ...
    /**
     * Método principal que executa o sistema com integração ao banco Oracle.
     *
    ...
    public static void main(String[] args) {
        ...
        System.out.println("      Backend Java + Oracle Database");
        System.out.println("    Versão 2.0 - Persistência Automática");
        System.out.println("==============================================\n");

        // Inicializa os serviços com integração ao banco
        inicializarSistema();

        // Testa a persistência de dados
        testarPersistenciaDados();

        // Executa as demonstrações apenas se necessário
        if (shouldRunDemonstrations()) {
    ...
    /**
     * Inicializa o sistema criando as instâncias dos serviços com integração Oracle.
     */
    private static void inicializarSistema() {
        System.out.println(">>> Inicializando Sistema com Banco Oracle...");

        try {
            // Inicializa o serviço principal (que automaticamente conecta ao Oracle)
            ...
            System.out.println("🔗 Integração com Oracle ativa");

        } catch (Exception e) {
            System.err.println("❌ Erro na inicialização: " + e.getMessage());
            System.out.println("🔄 Sistema funcionará em modo de memória");
        }
    ...
    /**
     * Testa a persistência de dados no banco Oracle.
     */
    private static void testarPersistenciaDados() {
        System.out.println("=== TESTE DE PERSISTÊNCIA NO ORACLE ===");

        // Verifica se já existem dados no banco
        int amostrasExistentes = service.listarSamples().size();
        int medicoesExistentes = service.listarMeasurements().size();
        int operadoresExistentes = service.listarOperators().size();

        System.out.println("📊 Dados carregados do banco:");
        System.out.println("   • Amostras: " + amostrasExistentes);
        System.out.println("   • Medições: " + medicoesExistentes);
        System.out.println("   • Operadores: " + operadoresExistentes);

        if (amostrasExistentes > 0 || medicoesExistentes > 0) {
            System.out.println("✅ SUCESSO: Dados foram recuperados após reinicialização!");
            System.out.println("💾 O sistema está persistindo corretamente no Oracle");
        } else {
            System.out.println("🆕 Nenhum dado encontrado - primeira execução ou banco vazio");
        }
    ...
    /**
     * Determina se deve executar as demonstrações completas.
     *
     * @return true se deve executar demonstrações
     */
    private static boolean shouldRunDemonstrations() {
        // Se já existem dados suficientes, pula as demonstrações
        int totalDados = service.listarSamples().size() +
                        service.listarMeasurements().size() +
                        service.listarOperators().size();

        if (totalDados > 5) {
            System.out.println("ℹ️  Dados suficientes encontrados - pulando demonstrações");
            return false;
        }

        System.out.println("🔄 Executando demonstrações para popular o banco...");
        return true;
    }
    ...
    /**
     * Finaliza a demonstração com um relatório final sobre persistência.
     */
    private static void finalizarDemonstracao() {
        ...
        int totalAmostras = service.listarSamples().size();
        int totalMedicoes = service.listarMeasurements().size();
        int totalOperadores = service.listarOperators().size();

        System.out.println("💾 Dados persistidos no Oracle:");
        System.out.println("   • " + totalAmostras + " amostras salvas");
        System.out.println("   • " + totalMedicoes + " medições salvas");
        System.out.println("   • " + totalOperadores + " operadores salvos");

        System.out.println("\n🔄 TESTE DE REINICIALIZAÇÃO:");
        System.out.println("1. Pare este programa (Ctrl+C)");
        System.out.println("2. Reinicie sua máquina");
        System.out.println("3. Execute novamente este programa");
        System.out.println("4. Verifique se os dados são recuperados automaticamente");

        System.out.println("\n--- Próximos Passos de Integração ---");
        System.out.println("1. Executar módulo Python de visão computacional");
        System.out.println("2. Executar API REST (RestApiController) na porta 8080");
        System.out.println("3. Abrir dashboard web para visualização");
        System.out.println("4. Testar integração completa Python → Java → Oracle → Web");
        ...
        System.out.println("- URL: oracle.fiap.com.br:1521:orcl");
        System.out.println("- Schema: RM555211");
        System.out.println("- Tabelas criadas conforme oracle_schema.sql");

        System.out.println("\n==============================================");
        System.out.println("    ✅ SISTEMA COM PERSISTÊNCIA ATIVA");
        System.out.println("     🔗 Dados salvos no Oracle Database");
        System.out.println("     🔄 Recuperação automática funcional");
        System.out.println("==============================================");
    }
}