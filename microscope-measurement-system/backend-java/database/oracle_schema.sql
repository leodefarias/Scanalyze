...
CREATE TABLE TB_MICROSCOPY_IMAGES (
    ...
    CONSTRAINT CK_MICROSCOPY_IMAGES_FORMATO CHECK (FORMATO IN ('JPEG', 'JPG', 'PNG', 'TIFF', 'BMP'))
);
...
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.NOME_ARQUIVO IS 'Nome do arquivo de imagem';
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.IMAGEM_BLOB IS 'Conteúdo binário da imagem armazenado como BLOB';
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.RESOLUCAO IS 'Resolução da imagem (ex: 1920x1080)';
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.TAMANHO_ARQUIVO IS 'Tamanho do arquivo em bytes';
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.FORMATO IS 'Formato da imagem (JPEG, PNG, etc.)';
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.DATA_CRIACAO IS 'Data de criação do registro';
COMMENT ON COLUMN TB_MICROSCOPY_IMAGES.OBSERVACOES IS 'Observações sobre a imagem';
